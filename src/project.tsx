...
function* runLayer(view: any, item: any, refs: any) {
  ...
  if (item.type === 'image') {
    ...
    if (wipeLeft) {
      ...
      // Invisible probe to measure natural rendered size
      view.add(
        <Img
          ref={imgRef2}
          src={item.src}
      ...
      // Clipping mask — starts at full width on the RIGHT side, shrinks leftward
      view.add(
        <Rect
          ref={maskRef}
          x={rawX + realW / 2}   // right-aligned anchor
          y={rawY}
          width={0}
          height={realH * 1.05}
          clip={true}
        ...
        >
          <Img
            ...
            src={item.src}
            width={imgWidth}
            x={-realW / 2}        // keep image pinned to right edge of mask
            y={0}
            zIndex={item.zIndex ?? 1}
          />
        </Rect>
      ...
      // Reveal: expand mask from right to left
